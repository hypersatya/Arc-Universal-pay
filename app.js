...
const operators = {
    ...
    broadband: ["Airtel Xstream", "JioFiber", "ACT Fibernet", "Hathway"],// --- CONFIGURATION ---
const USDC_ADDR = "0x3600000000000000000000000000000000000000";
const MERCHANT = "0xbdc55a1296d065b7eb4363207d1a599e578712c5"; // New Address Updated
const ARC_CHAIN = "0x4cef52";
...
const labels = {
    ...
    train: "PNR No", bus: "Route Details", flight: "Destination", movie: "Cinema Name"
};
...
function loadPlans() {
    ...
    mockPlans.forEach(plan => {
        ...
        list.innerHTML += `<div onclick="selectPlan(${plan.inr}, ${usdc})" class="plan-card p-4 rounded-xl mb-2 flex justify-between items-center text-[11px]">
...
async function executeFinalPayment() {
    ...
    try {
        ...
        await tx.wait();
        alert("Payment Sent to Merchant! ✅");
...
async function getHistory(limit, targetId) {
    ...
    try {
        ...
        list.innerHTML = logs.slice(-limit).reverse().map(l => `<div class="flex justify-between border-b border-white/5 pb-3 text-[10px]">
